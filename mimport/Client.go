 ...
 */
type MIMPORT_IMPL struct {
     config configuration.CONFIGURATION
}

/**
 * Import config into local cluster.
 * @param    *models.ImportConfigurations        body     parameter: Required
 * @return	Returns the  response from the API call
 */
func (me *MIMPORT_IMPL) UpdateImportConfig (
            body *models.ImportConfigurations) (error) {
//validating required parameters
    if (body == nil){
        return errors.New("The parameter 'body' is a required parameter and cannot be nil.")
}     //the endpoint path uri
    _pathUrl := "/public/import/config"
